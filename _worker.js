...
// **PONTO CRÍTICO**: export async function onRequest — Pages Functions detecta isso
export async function onRequest(context) {
  ...
  if (path.startsWith('api/')) {
    ...
    if (meth==='GET' && path==='api/list') {
      ...
      const items = await Promise.all(
        list.keys.filter(k=>/^[a-z0-9]{6}$/i.test(k.name)).map(async k=>{
  ...
  if (meth==='POST') {
    try {
      ...
      const meta = {
        created: Date.now(),
        creator:{ ip:request.headers.get('CF-Connecting-IP'), loc:request.cf?.country||'??' },
