...
export default {
  async fetch(request, env) {
    ...
    // ─── Fallback: serve estáticos de public/ ───
...
async function handleRedirect(request, env, slug) {
  ...
  const logsArr = JSON.parse((await env.LOGS.get(logsKey)) || '[]');
  logsArr.unshift({
    t: Date.now(),
  ...
  env.LOGS.put(logsKey, JSON.stringify(logsArr));
...
async function handleApi(request, env, parts) {
  ...
  if (request.method === 'GET' && action === 'list') {
    const list = (await env.LINKS.list()).keys.filter(k => /^[a-z0-9]{6}$/i.test(k.name));
  ...
  if (request.method === 'GET' && action === 'detail' && slug) {
    ...
    return new Response(
      JSON.stringify({ total, byDay, byHour, points, logs: rawLogs.slice(0, 100) }),
      { headers: { 'Content-Type': 'application/json' } }
    );
  }
